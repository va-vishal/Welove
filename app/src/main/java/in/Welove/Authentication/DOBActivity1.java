...
public class DOBActivity1 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        cardView2.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                CustomDatePickerBottomSheet dialog = new CustomDatePickerBottomSheet(DOBActivity1.this, new CustomDatePickerBottomSheet.OnDateSetListener() {
                    @Override
        ...
        saveButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                // Gather user profile data from input fields
                ...
                if (validateFields(name, description, gender, educationLevel, jobType, motherTongue, maritalStatus, prefGender)) {
                    ...
                    userProfile.put("p", false);  // Profile completion status
                    userProfile.put("pp", false); // Some other profile-related flag
                    ...
                    // Set the "completeprofile" field to true after successfully completing the profile
                    userProfile.put("completeprofile", true);

                    FirebaseAuth auth = FirebaseAuth.getInstance();
                    FirebaseUser user = auth.getCurrentUser();

                    if (user != null) {
                        // Database reference to save user data
                        ...
                        reference.updateChildren(userProfile)
                                .addOnSuccessListener(aVoid -> {
                                    // Send email verification after profile update
                                    user.sendEmailVerification()
                                            .addOnCompleteListener(emailTask -> {
                                                if (emailTask.isSuccessful()) {
                                                    // Call image upload function (if needed)
                                                    uploadImage();
                                                    showSuccessDialog();  // Show success dialog if needed
                                                } else {
                                                    Toast.makeText(DOBActivity1.this, "Failed to send verification email: " + emailTask.getException().getMessage(), Toast.LENGTH_SHORT).show();
                                                }
                                            });
                                })
                                .addOnFailureListener(e -> {
                                    // Handle failure to update user profile
                                    Toast.makeText(DOBActivity1.this, "Failed to update user's profile: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                                });
                    }
                } else {
                    // If validation fails, show a message
                    Toast.makeText(DOBActivity1.this, "Please fill in all required fields.", Toast.LENGTH_LONG).show();
                }
            }
        });
    ...
    private void showSuccessDialog() {
        CustomDialog customDialog = new CustomDialog(DOBActivity1.this);
        customDialog.show(
                "Congratulations!",
                "! Your account has been successfully created. Please Login to get started.",
                v -> {
                    customDialog.dismiss();
                    Intent homeIntent = new Intent(DOBActivity1.this, LoginActivity1.class);
    ...
    private void getCurrentLocation() {
        ...
        timeoutRunnable = new Runnable() {
            ...
            public void run() {
                hideLoadingDialog();
                Toast.makeText(DOBActivity1.this, "Location retrieval timed out. Please try again.", Toast.LENGTH_SHORT).show();
            }
        };
        ...
        fusedLocationClient.getLastLocation()
                .addOnSuccessListener(this, location -> {
                    ...
                    if (location != null) {
                        ...
                        Toast.makeText(DOBActivity1.this, "Location obtained", Toast.LENGTH_SHORT).show();
                    } else {
                        Toast.makeText(DOBActivity1.this, "Unable to get location", Toast.LENGTH_SHORT).show();
                    }
                })
                .addOnFailureListener(e -> {
                    hideLoadingDialog(); // Hide loading dialog on failure
                    handler.removeCallbacks(timeoutRunnable); // Remove timeout
                    Toast.makeText(DOBActivity1.this, "Failed to get location: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                });
    }
    ...
    private void showMediaPickerDialog() {
        AlertDialog.Builder builder = new AlertDialog.Builder(DOBActivity1.this);
    ...
    private void handleImageSelectionOrCapture(int requestCode, Intent data) {
        if (requestCode == PICK_IMAGE_REQUEST && data != null && data.getData() != null) {
            ...
            if (imageBitmap != null) {
                mediaUri = getImageUri(DOBActivity1.this, imageBitmap);
            }
        }
        if (mediaUri != null) {
        ...
        if (mediaUri != null) {
            ...
            uploadTask.continueWithTask(task -> {
                ...
                if (task.isSuccessful()) {
                    ...
                    Toast.makeText(DOBActivity1.this, "Failed to upload image", Toast.LENGTH_SHORT).show();
                    Log.e("ProfilePhotoActivity", "Upload failed: ", task.getException());
                }
            }).addOnFailureListener(e -> {
                Toast.makeText(DOBActivity1.this, "Upload failed: " + e.getMessage(), Toast.LENGTH_SHORT).show();
    ...
    private void saveImageToDatabase(String imageurl) {
        ...
        if (user != null) {
            ...
            reference.updateChildren(hashMap).addOnCompleteListener(task -> {
                if (task.isSuccessful()) {
                    Toast.makeText(DOBActivity1.this, "Profile image uploaded", Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(DOBActivity1.this, "Failed to update profile image", Toast.LENGTH_SHORT).show();
                }
            }).addOnFailureListener(e -> {
                Toast.makeText(DOBActivity1.this, "Error: " + e.getMessage(), Toast.LENGTH_SHORT).show();
            });
        }
    }
